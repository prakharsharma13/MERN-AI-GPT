...
**Nexus AI** is a full-stack prototype that pairs a React + Vite client with an Express/MongoDB server to provide a chat experience backed by the OpenAI API and auxiliary services (image uploads, payments, webhooks). It is structured for local development and easy deployment (Vercel-ready configuration present).
...
Create a `.env` file for the `server` service. Exact names are used inside the config files in [server/configs](server/configs). Typical variables include:
...
- See [server/package.json](server/package.json) for the server scripts (`server`, `start`).
- See [client/package.json](client/package.json) for the client scripts (`dev`, `build`, `preview`, `lint`).
...
- The repository contains Vercel configuration files (`vercel.json`) in relevant folders and is prepared for a typical Vercel deployment. Configure environment variables in your Vercel project settings and point the client and server deployments appropriately.
...
- Keep changes focused; add or update documentation when adding new features.
...
- Want me to add an example `.env.example` with the exact environment variable names used by the server? I can create one based on the files under [server/configs](server/configs).
